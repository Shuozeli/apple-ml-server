...
final class SpeechWorker: @unchecked Sendable {
    ...
    private static func runRecognition(
        ...
        let task = sr.recognitionTask(with: request) { result, error in
            ...
            partialConfidence = confidences.isEmpty ? 0 : confidences.reduce(0, +) / Float(confidences.count)
        ...
        if !isDone {
            ...
            if !partialTranscript.isEmpty {
                continuation.resume(returning: (partialTranscript, partialConfidence, nil))
            } else {
